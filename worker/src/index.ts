...
<body>
  <header>
    <div class="head">
      <div><h1>ChorusDBJP</h1><p>日本の合唱曲を、編成・難易度・作者から探せるデータベース</p></div>
      <div id="total" class="count-badge">読込中</div>
    </div>
  </header>
  <main>
  ...
  <dialog id="detail">
    <div class="modal-head"><h2 id="detail-title"></h2><button id="close" class="close" type="button" aria-label="閉じる">×</button></div>
  ...
  <script>
    ...
    function card(song) {
      ...
      const tags = [
        ...
        song.difficulty ? '<span class="tag difficulty" aria-label="難易度 ' + esc(song.difficulty) + '">' + stars(song.difficulty) + '</span>' : ''
      ].join('');
      return '<article class="card" tabindex="0" role="button" data-id="' + esc(song.id) + '">' +
        '<span class="arrow">›</span><h2>' + esc(song.title) + '</h2>' +
    ...
    function currentItems() {
      ...
      const items = state.songs.filter(function (song) {
        const haystack = [song.title, song.title_kana, song.lyricist, song.composer, song.arranger].join(' ').toLowerCase();
        return (!q || haystack.includes(q)) && (!v || song.voicing === v) && (!d || String(song.difficulty) === d);
      });
      items.sort(function (a, b) {
        if (sort.value === 'difficulty-asc') return (a.difficulty || 99) - (b.difficulty || 99);
        if (sort.value === 'difficulty-desc') return (b.difficulty || 0) - (a.difficulty || 0);
        if (sort.value === 'composer') return String(a.composer || '').localeCompare(String(b.composer || ''), 'ja');
        return String(a.title_kana || a.title).localeCompare(String(b.title_kana || b.title), 'ja');
      });
      return items;
    }
    ...
    function render() {
      ...
      status.textContent = state.songs.length + '曲中 ' + items.length + '曲を表示';
      results.innerHTML = items.length ? items.map(card).join('') : '<div class="empty">条件に合う曲がありません。<br>検索条件を変えてみてください。</div>';
    }

    function detailRows(song) {
      const rows = [
        ['読み', song.title_kana], ['作詞', song.lyricist], ['作曲', song.composer], ['編曲', song.arranger],
        ['編成', song.voicing], ['伴奏', song.accompaniment], ['演奏時間', song.duration_seconds ? Math.floor(song.duration_seconds / 60) + '分' + String(song.duration_seconds % 60).padStart(2, '0') + '秒' : null],
        ['難易度', song.difficulty ? stars(song.difficulty) + ' (' + song.difficulty + '/5)' : null], ['出版社', song.publisher]
      ];
    ...
    function openDetail(id) {
      ...
      detailBody.innerHTML = '<div class="empty">曲情報を取得しています</div>';
      detail.showModal();
      history.replaceState(null, '', '#song=' + encodeURIComponent(id));
      fetch('/api/songs/' + encodeURIComponent(id)).then(function (response) {
        if (!response.ok) throw new Error('曲情報を取得できませんでした');
        return response.json();
      }).then(function (song) {
        detailTitle.textContent = song.title;
        detailBody.innerHTML = '<dl class="detail-grid">' + detailRows(song) + '</dl>' +
          '<div class="notice">現在の収録情報には未検証データが含まれます。編成や難易度は楽譜版によって異なる場合があります。</div>';
      }).catch(function (error) {
        detailTitle.textContent = 'エラー';
    ...
    fetch('/api/songs').then(function (response) {
      ...
      total.textContent = state.songs.length + '曲収録';
      Array.from(new Set(state.songs.map(function (song) { return song.voicing; }).filter(Boolean))).sort().forEach(function (value) {
        const option = document.createElement('option'); option.value = value; option.textContent = value; voicing.appendChild(option);
      });
      render();
      const matched = location.hash.match(/^#song=(.+)$/);
      if (matched) openDetail(decodeURIComponent(matched[1]));
    }).catch(function (error) {
      total.textContent = '接続エラー';
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/api/health' && request.method === 'GET') {
      try {
        const row = await env.DB.prepare('SELECT COUNT(*) AS songs FROM works WHERE published = 1').first();
        return json({ ok: true, songs: Number(row?.songs || 0) });
      } catch (error) {
    ...
    if (url.pathname === '/api/songs' && request.method === 'GET') {
      try {
        ...
        if (voicing) { conditions.push('e.voicing = ?'); values.push(voicing); }
        if (difficulty >= 1 && difficulty <= 5) { conditions.push('e.difficulty = ?'); values.push(difficulty); }
    ...
    if (songMatch && request.method === 'GET') {
      try {
        const song = await env.DB.prepare(songSelect + ' WHERE w.published = 1 AND w.id = ? LIMIT 1').bind(songMatch[1]).first();
